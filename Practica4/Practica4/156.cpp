...
struct AnagramData
{
	...
	AnagramData() : originalInput() , isAnagram(false) {}
	AnagramData(string input) : originalInput(input) , isAnagram(false) {}
};

typedef Arbus<string,AnagramData> AnagramsTree;

#define TOLOWER_CHAR(x) 
...
int main()
{
	AnagramsTree searchTree;
	vector<string> results;
	Lista<AnagramData> treeNodes;
	...
	while(cin && input != "#")
	{
		...
		if(input != "#")
		{
			...
			if(!searchTree.esta(sortedInput))
				searchTree.inserta(sortedInput,AnagramData(input));
			else
				searchTree.consulta(sortedInput).isAnagram = true;
		}
	}

	for(AnagramsTree::Iterador it = searchTree.principio() ; it != searchTree.final() ; it.avanza())
		if(!it.valor().isAnagram)
			results.push_back(it.valor().originalInput);

	sort(results.begin() , results.end());

	for(unsigned int i = 0 ; i < results.size() ; ++i) //Viva C++ 11 !!!
		cout << results[i] << endl;
}